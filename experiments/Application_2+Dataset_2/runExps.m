...
% Dataset 1: 13-dimensional CyTOF Data
    ...
    setup();
    for i=1:2,
       [XTr, YTr, XTe, YTe] = load_data(i);
       runEuclidean(XTr, YTr, XTe, YTe, 1, i);
       runDMLMJ(XTr, YTr, XTe, YTe, 1, i);
    end
end
...
function [XTr, YTr, XTe, YTe] = load_data(num)
    % load training data
    data = importdata(['data/Levine_32dim_train_strat_patient' num2str(num) '.csv']);
    data = data.data;
    XTr = data(:,2:end-1)'; % the first column are indices
    YTr = data(:,end);
    
    % load test data
    data = importdata(['data/Levine_32dim_test_strat_patient' num2str(num) '.csv']);
    data = data.data;
